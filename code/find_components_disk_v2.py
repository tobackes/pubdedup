...
def find_subsup(start,end,search_size):
    ...
    for index1 in range(start,end+1): # What is the reason for the +1? I guess because the input range assume inclusive interval
        if len(C[index1-start]) >= search_size:
            break;
        feats      = [feat for freq,feat in sorted([(len(S[feat]),feat,) if feat in S else (0,feat,) for feat in C[index1-start]])];
...
def get_min_el_map(tasks,cur,sizes,boundaries,num_workers=16): #Assuming that sizes is smallest first!
    ...
    S                              = get_structOf(cur,manager,patch[0],patch[1]);
    for i in range(1,len(tasks)): #TODO: Improve the granularity of what objects use how much of ram, in particular the results!
        ...
        S                    = get_structOf(cur,manager,patch[0],patch[1]);                 row += [(time.time()-row[5])];
...
cur_out.execute("CREATE TABLE components(label INT, repIDIndex INTEGER PRIMARY KEY)");
cur_out.executemany("INSERT INTO components VALUES(?,?)",((label,repIndex,) for label in range(len(components)) for repIndex in components[label] if len(components[label]) > 1)); #TODO: Can be changed
