...
columns = [row[1] for row in cur.execute("PRAGMA table_info(mentions)").fetchall()];

num_feats = { row[0]: sum([cell != None for cell in row[2:]]) for row in cur.execute("SELECT * FROM mentions") };

def selector(num_feats,cur):
    for repID in num_feats:
        if num_feats[repID] <= _max_feats:
            row    = cur.execute('SELECT * FROM mentions WHERE repID=?',(repID,)).fetchall()[0];
            freq   = int(row[1]);
            #counts = [];
            values = [];
            ...
            for i in range(2,len(columns)):
                if row[i] != None:
                    #count = cur.execute('SELECT COUNT(*) FROM mentions WHERE '+' OR '.join([columns[i][:-1]+str(j)+'="'+row[i]+'"' for j in range(1,5)])).fetchall()[0][0];
                    #counts.append(count);
            ...
            query = 'SELECT COUNT(*) FROM mentions WHERE ('+') AND ('.join(ors)+')';
...
for attribute,repID in specifications:
    print(       'UPDATE mentions SET '+attribute+'="'+repID+'" WHERE repID="'+repID+'"');
    cur_.execute('UPDATE mentions SET '+attribute+'="'+repID+'" WHERE repID="'+repID+'"');
    con_.commit();
