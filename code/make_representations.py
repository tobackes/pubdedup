...
_selection     = [5,6,7,8,10,11,12,14,15,16,18,19,20,22,23,24,25]; #TODO: Load this from a mapping file!
...
def bundle(row):
    groups = [[6,10,14,18],[7,11,15,19],[8,12,16,20],[22,23,24,25]];
...
def initialize_primary_keys(cur_in,cur_out,con_out):
    mention        = cur_in.execute("SELECT * FROM publications ORDER BY mentionID LIMIT 1").fetchall()[0];
...
def insert_representations(Q,cur_in,cur_out,con_out):
    for start,size in Q:
        ...
        #------------------------------------------------------------------------------------------------------------------------------------
        mentions        = cur_in.execute("SELECT * FROM publications ORDER BY mentionID LIMIT ?,?",(start,size,)).fetchall();
...
num_rows = cur_in.execute("SELECT count(*) FROM publications").fetchall()[0][0];
