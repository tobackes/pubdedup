...
for v_initial,v_firstname,v_term,v_year in inputs:
    ...
    if OUT and len(solutions)>0:
        for v_title in range(n_title+1):
            for v_newfirst in range(4+1):
                OUT.write(str(v_initial)+' surname '+str(v_firstname)+' initial '+str(v_newfirst)+' first '+str(v_term)+' term '+str(v_year)+' year '+str(v_title)+' title --> '+' | '.join( (str(solution[-1][0])+' surname '+str(solution[-1][1])+' initial 0 first '+str(solution[-1][2])+' term '+str(solution[-1][3])+' year 0 title' for solution in solutions))+'\n' );

    print(str(v_initial)+' surname '+str(v_firstname)+' initial '+str(v_term)+' term '+str(v_year)+' year --> '+' | '.join( (str(rhs[0])+' surname '+str(rhs[1])+' initial '+str(rhs[2])+' term '+str(rhs[3])+' year'+' ['+str(int(objective))+'/'+str(int(footprint))+']' for objective,footprint,rhs in solutions) )+'   ('+str(sum((footprint for _,footprint,_ in solutions)))+')' );
